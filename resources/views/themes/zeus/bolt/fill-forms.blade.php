@php
    $colors = \Illuminate\Support\Arr::toCssStyles([
        \Filament\Support\get_color_css_variables(
            $zeusForm->options['primary_color'] ?? 'primary',
...
<div class="not-prose" style="{{ $colors }}">
    ...
    @if (!$inline)
        @if (
            !class_exists(\LaraZeus\BoltPro\BoltProServiceProvider::class) ||
                (optional($zeusForm->options)['logo'] === null && optional($zeusForm->options)['cover'] === null))
            <x-slot name="header">
                <h2>{{ $zeusForm->name ?? '' }}</h2>
        ...
        <x-slot name="breadcrumbs">
            ...
            @else
                <li class="flex items-center">
                    <a
                        href="{{ \LaraZeus\Bolt\Facades\Extensions::init($zeusForm, 'route') }}">{{ \LaraZeus\Bolt\Facades\Extensions::init($zeusForm, 'label') }}</a>
                    @svg('iconpark-rightsmall-o', 'fill-current w-4 h-4 mx-3 rtl:rotate-180')
                </li>
            @endif
            <li class="flex items-center">
    ...
    @if (class_exists(\LaraZeus\BoltPro\BoltProServiceProvider::class) &&
        ...
        <div style="background-image: url('{{ \Illuminate\Support\Facades\Storage::disk(config('zeus-bolt.uploadDisk'))->url($zeusForm->options['cover']) }}')"
            ...
            <div>
                <img class="bg-white rounded-full shadow-md shadow-custom-100 sm:w-24 object-cover"
